...
function toggleMute(button) {
    ...
    const soundWaves = icon.querySelectorAll('.sound-wave');

    if (video.muted) {
        video.muted = false;
        button.classList.add('unmuted');
        // Show sound waves for unmuted state
        ...
        // Hide sound waves for muted state
        soundWaves.forEach(wave => {
...
/* ===== INITIALIZE MUTE BUTTONS ON LOAD ===== */
document.addEventListener('DOMContentLoaded', () => {
    // Ensure all videos start muted and sound waves are hidden
    document.querySelectorAll('.mute-button').forEach(button => {
        const icon = button.querySelector('.mute-icon');
        const soundWaves = icon.querySelectorAll('.sound-wave');
        
        // Initially hide sound waves (muted state)
        soundWaves.forEach(wave => {
            wave.style.display = 'none';
        });
    });
